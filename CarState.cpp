...
void 
CarState::setAll(string sensors)
{
        SimpleParser::parse(sensors, "angle", this->angle);
        SimpleParser::parse(sensors, "curLapTime", this->curLapTime);
        SimpleParser::parse(sensors, "damage", this->damage);
...
void CarState::createImage(int width, int height)
{
	if(foresightImg!=NULL){
		cvReleaseImage(&foresightImg);
	}

	if(backsightImg!=NULL){
		cvReleaseImage(&backsightImg);
	}
	//creat image
	foresightImg = cvCreateImage(cvSize(width, height), IPL_DEPTH_8U, 3);
	backsightImg = cvCreateImage(cvSize(width, height), IPL_DEPTH_8U, 3);
}

void CarState::releaseImage()
{
	if(foresightImg!=NULL){
		cvReleaseImage(&foresightImg);
	}
...
void CarState::saveImage(const char camID, const unsigned char *imgBuf, int width, int height)
{
	IplImage *tImg=NULL;

	switch(camID)
	{	
	case 0: tImg = foresightImg; break;
	case 1: tImg = backsightImg; break;
	}

	if(tImg!=NULL)
	{
		for (int i = (height-1), j=0; i >=0 ; i--,j++) {
			memcpy((unsigned char*)(tImg->imageData + i * tImg->widthStep), imgBuf + j * width * 3, width * 3);
		}
		//save image to disk
		//char imgFileName[20];
		//sprintf(imgFileName,"/tmp/receive%c.png",camID);
		//cvSaveImage(imgFileName,tImg);
	}
}

IplImage* CarState::getForesightImg()
{
	    return foresightImg;
}

IplImage* CarState::getBacksightImg()
{
		return backsightImg;
}

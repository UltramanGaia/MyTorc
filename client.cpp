...
//    	cout << "SEED: " << seed << endl;
	...
	switch(serverPort)
	{
	case 3001: 	d = new QuickBasicDriver; break;
	case 3002: 	d = new QuickAdvancedDriver; isImageRace=true; break;				
	case 3003: 	d = new FollowDriver; break;
	case 3004: 	d = new OvertakeDriver; break;
	default: d = new tDriver; break;	
	}
            ...
            if (select(socketDescriptor+1, &readSet, NULL, NULL, &timeVal))
            {
                ...
                if(strncmp(buf,"cam",3)==0) 
				{
					if(isImageRace==true)
					{
						...
						//process image.
						// ...
						cs.saveImage(buf[3],(unsigned char *)(buf+4),200,100);
					}
                    continue;
                }
				...
				{
					cs.setAll(string(buf));
					//string action = d->drive(string(buf));
					string action = d->wDrive(cs).toString();
